...
import type { AgentState, DispatchLogEntry, DispatchRule, Message, Task, TeamConfig, TeamMember } from './types/index';
      ...
      'add-dispatch-rule': tool({
        ...
        args: {
          ...
          rule: tool.schema.object({
            ...
            eventType: tool.schema.string().describe('Event type to trigger on (e.g. task.created)'),
            condition: tool.schema.object({
              type: tool.schema.string().describe('Condition type (simple_match, resource_count)'),
              ...
              value: tool.schema.union([tool.schema.string(), tool.schema.number(), tool.schema.boolean()]).describe('Value to compare against'),
            }).optional(),
